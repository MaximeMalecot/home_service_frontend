...
<body>
    ...
    <?php
      ...
      \Stripe\Stripe::setApiKey('sk_test_qMXWSSMoE6DTqXNR7kMQ0k6V00sh4hnDbe');
			if(isset($_SESSION['mail']) && isset($_GET['object']) && isset($_GET['session_id']) && isset($_GET['devis'])){
        try{
          ...
          $reqDevis=$cx->prepare('DELETE FROM devis WHERE idDevis = ?');

          $rez = unserialize($_GET['object']);
          ...
          $reqDevis->execute(array(
            $_GET['devis']
					...
					echo "<h1>Votre réservation a bien été prises en compte, merci de votre confiance !</h1>";
